...
export default function UserSearch({ posts, onSelect }) {
  ...
  return (
    <div className="relative w-full sm:max-w-sm">
      ...
      <input
        id="user-search"
        className="field w-full py-2 pl-9 text-sm sm:py-2.5 sm:pl-10"
        value={query}
        onChange={(event) => setQuery(event.target.value)}
        placeholder="Search people in the feed…"
      />
      <span className="pointer-events-none absolute left-3 top-2.5 text-slate-400 sm:left-4 sm:top-3">
        ⌕
      </span>
      {query.trim() && (
        <div className="absolute z-20 mt-2 w-full max-w-[calc(100vw-1.5rem)] overflow-hidden rounded-2xl border border-slate-100 bg-white p-2 shadow-card sm:max-w-none">
          {users.length ? (
            users.map((user) => (
              <button
                key={user.userId}
                type="button"
                className="flex w-full items-center gap-3 rounded-xl p-2 text-left hover:bg-violet/5"
                onClick={() => {
                  onSelect(user);
                  setQuery("");
                }}
              >
                <Avatar name={user.username} onClick={() => {}} />
                <span className="truncate font-bold">{user.username}</span>
              </button>
            ))
          ) : (
            <p className="px-3 py-3 text-sm text-slate-500">
              No matching authors in the loaded feed.
            </p>
          )}
        </div>
      )}
